...
class PlaylistTest {
    ...
    void testDefaultConstructor() {
        ...
        assertTrue(empty.getMusics().isEmpty());
    }

    @Test
    void testSetMusics() {
        Playlist p = new Playlist("P", "A");
        List<Music> toAdd = new ArrayList<>();
        toAdd.add(music1);
        toAdd.add(music2);
        p.setMusics(toAdd);
    ...
    void testSetId() {
        playlist.setId(999);
        assertEquals(999, playlist.getId());
    }
    ...
    void testEqualsDifferentType() {
        assertNotEquals(playlist, "string");
    }

    @Test
    void testConstructorWithNullMusics() {
        Playlist p = new Playlist("NullTest", "Author", null);
        assertEquals("NullTest", p.getName());
        assertTrue(p.getMusics().isEmpty());
    }

    @Test
    void testTwoPlaylistsHaveDifferentIds() {
        Playlist p1 = new Playlist("P1", "A");
        Playlist p2 = new Playlist("P2", "B");
        assertNotEquals(p1.getId(), p2.getId());
    }

    @Test
    void testGetIdIsNotNegative() {
        Playlist p = new Playlist("P", "A");
        assertTrue(p.getId() >= 0);
    }

    @Test
    void testEqualsReturnsFalseWhenMusicsDiffer() {
        Playlist other = new Playlist("Queen Hits", "Fan", List.of(music1, music2));
        assertNotEquals(playlist, other);
    }

    @Test
    void testEqualsReturnsFalseWhenNameDiffers() {
        Playlist other = new Playlist("Different Name", "Fan", List.of(music1));
        assertNotEquals(playlist, other);
    }

    @Test
    void testEqualsReturnsFalseWhenAutorDiffers() {
        Playlist other = new Playlist("Queen Hits", "OtherFan", List.of(music1));
        assertNotEquals(playlist, other);
    }

    @Test
    void testGetMusicByIndexLastValidIndex() throws AlreadyExistsException {
        playlist.addMusic(music2);
        assertEquals(music2, playlist.getMusicBYIndex(1));
    }

    @Test
    void testGetMusicByIndexSizeThrows() throws AlreadyExistsException {
        playlist.addMusic(music2);
        assertThrows(IndexOutOfBoundsException.class, () -> playlist.getMusicBYIndex(2));
    }

    @Test
    void testGetMusicByIndexNegativeThrows() {
        assertThrows(IndexOutOfBoundsException.class, () -> playlist.getMusicBYIndex(-1));
    }

    @Test
    void testSequentialIds() {
        Playlist p1 = new Playlist("SeqA", "a");
        Playlist p2 = new Playlist("SeqB", "b");
        assertEquals(p1.getId() + 1, p2.getId());
    }

    @Test
    void testSequentialIdsFullConstructor() {
        Playlist p1 = new Playlist("SeqC", "a", new ArrayList<>());
        Playlist p2 = new Playlist("SeqD", "b", new ArrayList<>());
        assertEquals(p1.getId() + 1, p2.getId());
    }

    @Test
    void testCopyConstructorAdvancesIdCounter() {
        Playlist p1 = new Playlist("OrigX", "a");
        Playlist copy = new Playlist(p1);
        Playlist p2 = new Playlist("AfterCopy", "a");
        // copy constructor must increment idCounter; p2 must come after both p1 and copy
        assertTrue(p2.getId() > p1.getId() + 1);
    }

    @Test
    void testEmptyConstructorAdvancesIdCounter() {
        Playlist p1 = new Playlist("BeforeEmpty", "a");
        Playlist empty = new Playlist();
        Playlist p2 = new Playlist("AfterEmpty", "a");
        assertEquals(empty.getId() + 1, p2.getId());
    }
}