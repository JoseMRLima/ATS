...
class AlbumTest {
    ...
    @Test
    void setCurrentSongNoArg_SetsOneOfTheSongs() {
        album.setCurrentSong();
        Song current = album.getCurrentSong();
        assertNotNull(current);
        assertTrue(album.getSongsCopy().stream().anyMatch(s -> s.getName().equals(current.getName())));
    }

    @Test
    void nextShuffle_ChangesSongWhenMultiple() {
        album.setCurrentSong(song1);
        album.nextShuffle();
        assertNotEquals(song1.getName(), album.getCurrentSong().getName());
    }

    @Test
    void nextShuffle_SingleSongNoChange() {
        Album single = new Album("Single", "Art", 2020, "Rock", new ArrayList<>(Arrays.asList(song1)));
        single.setCurrentSong(song1);
        single.nextShuffle();
        assertEquals(song1.getName(), single.getCurrentSong().getName());
    }
    ...
    @Test
    void previous_PremiumWrapsFromFirstToLast() throws Exception {
        album.setCurrentSong(song1);
        album.previous(premiumUser);
        assertEquals(song2.getName(), album.getCurrentSong().getName());
    }
    ...
    @Test
    void next_SingleSongFreeUserNoChange() {
        Album single = new Album("Single", "Art", 2020, "Rock", new ArrayList<>(Arrays.asList(song1)));
        single.setCurrentSong(song1);
        single.next(freeUser);
        assertEquals(song1.getName(), single.getCurrentSong().getName());
    }

    @Test
    void deleteSong_CurrentSongBecomesNull() {
        album.setCurrentSong(song1);
        album.deleteSong("Song1");
        assertFalse(album.getSongsCopy().stream().anyMatch(s -> s.getName().equals("Song1")));
    }

    @Test
    void getTotalDuration_MatchesSumOfSongs() {
        int expected = song1.getDurationInSeconds() + song2.getDurationInSeconds();
