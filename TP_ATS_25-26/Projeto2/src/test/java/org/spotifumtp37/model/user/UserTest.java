...
class UserTest {
    ...
    @Test
    void addPoints_FreePlanAddsExactlyFive() {
        User u = new User("U", "u@e.com", "addr", new FreePlan(), "pw", 0, new ArrayList<>());
        u.addPoints();
        assertEquals(5.0, u.getPontos(), 0.001);
    }

    @Test
    void addPoints_PremiumBaseAddsExactlyTen() {
        User u = new User("U", "u@e.com", "addr", new PremiumBase(), "pw", 0, new ArrayList<>());
        u.addPoints();
        assertEquals(10.0, u.getPontos(), 0.001);
    }

    @Test
    void addPoints_AccumulatesCorrectly() {
        User u = new User("U", "u@e.com", "addr", new FreePlan(), "pw", 0, new ArrayList<>());
        u.addPoints();
        u.addPoints();
        assertEquals(10.0, u.getPontos(), 0.001);
    ...
    @Test
    void updateHistory_IncreasesCountByOne() {
        int before = user.getHistory().size();
        user.updateHistory(song1);
        assertEquals(before + 1, user.getHistory().size());
    }

    @Test
    void updateHistory_StoresCorrectSong() {
        user.updateHistory(song1);
        List<History> h = user.getHistory();
        assertEquals(song1.getName(), h.get(h.size() - 1).getSong().getName());
    }

    @Test
    void updateHistory_MultipleCallsAccumulate() {
        user.updateHistory(song1);
        user.updateHistory(song2);
        assertEquals(2, user.getHistory().size());
    }

    @Test
    void updatePremiumTop_AddsExactly100Points() {
        User u = new User("U", "u@e.com", "addr", new PremiumBase(), "pw", 50, new ArrayList<>());
        u.updatePremiumTop(new PremiumTop());
        assertEquals(150.0, u.getPontos(), 0.001);
        assertTrue(u.getSubscriptionPlan() instanceof PremiumTop);
    }

    @Test
    void clone_ProducesEqualButDistinctObject() {
        User cloned = user.clone();
        assertEquals(user.getName(), cloned.getName());
        assertEquals(user.getPontos(), cloned.getPontos(), 0.001);
    ...
    void getHistory_ReturnsDefensiveCopy() {
        user.updateHistory(song1);
        List<History> h1 = user.getHistory();
        List<History> h2 = user.getHistory();
        assertNotSame(h1, h2);
        assertEquals(h1.size(), h2.size());
    }

    @Test
    void createTopGenrePlaylistWithinTime_ExactBoundary() {
        user.updateHistory(song1);
        List<Song> result = user.createTopGenrePlaylistWithinTime(albumMap, song1.getDurationInSeconds());
        assertEquals(1, result.size());
    }

    @Test
    void createTopGenrePlaylistWithinTime_OneLessThanDuration() {
        user.updateHistory(song1);
        List<Song> result = user.createTopGenrePlaylistWithinTime(albumMap, song1.getDurationInSeconds() - 1);
        assertTrue(result.isEmpty());
    }

    @Test
    void getTopGenre_NullHistoryReturnsNull() {
        User u = new User("U", "u@e.com", "addr", new FreePlan(), "pw", 0, new ArrayList<>());
        assertNull(u.getTopGenre());
    }

    @Test
    void equals_SameNameReturnsTrue() {
        User other = new User("TestUser", "other@e.com", "other addr", new PremiumBase(), "pw2", 999, new ArrayList<>());
        assertEquals(user, other);
    }

    @Test
    void equals_DifferentNameReturnsFalse() {
        User other = new User("Different", "test@example.com", "123 Main St", new FreePlan(), "password123", 0, new ArrayList<>());
        assertNotEquals(user, other);
    }

    @Test
    void createTopGenrePlaylist_EmptyHistoryReturnsEmpty() {
        List<Song> result = user.createTopGenrePlaylist(albumMap);
        assertTrue(result.isEmpty());
    }

    @Test
    void createTopGenreExplicitPlaylist_NoExplicitSongsInTopGenre() {
        user.updateHistory(song2);
        List<Song> result = user.createTopGenreExplicitPlaylist(albumMap);
        assertTrue(result.isEmpty());
    }
}